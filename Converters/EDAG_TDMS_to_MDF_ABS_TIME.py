...
class EDAG_TDMS_to_MDF_ABS_time:
    @staticmethod
        ...
        for input_file in input_file_list:
            ...
            for group in tdms_file.groups():
                if verbose:
                    ...
                    print([channel.name for channel in group.channels()])
                for channel in group.channels():
                    ...
                    if first_samples_extracted:
                        ...
                        if TDMS_real_number_of_sample != df_channel.shape[0] and df_channel.shape[0] != 6917:
                            print(
                            ...
                            if df_channel.shape[0] % TDMS_real_number_of_sample == 0:
                                ...
                                print(f" WARNING : Channel {channel.name} discarded")
                    ...
                    '''
                    if channel_to_keep:
                        if df_channel.shape[0] == 6917:
                            rows_to_add = 6924 - 6917
                            padding = pd.DataFrame(0, index=range(rows_to_add), columns=df_channel.columns)
                            df_channel = pd.concat([df_channel, padding], ignore_index=True)
            ...
            if "DateTime" in final_df.columns:
                final_df['Time[s]'] = (final_df['DateTime'] - final_df['DateTime'].iloc[0]).dt.total_seconds()

            # Convert timedelta64 to seconds
            #timestamps = final_df['Time[s]'] / np.timedelta64(1, 's')

            #print(final_df['Time[s]'])
            ...
            #if "DateTime" in final_df.columns:
            timestamps = np.array(final_df['Time[s]'])
            #else:
            #timestamps = np.arange(start=0, stop=TDMS_duration, step=TDMS_increment)
